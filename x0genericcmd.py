...
class X0HDFuryVertex2Cmd(X0GenericCmd):
    """
...
class X0OnkyoReceiverCmd(X0GenericCmd):
    """
    ...
    def makeDesired(self, cmd, data):
        ...
        log.debug(f'makeDesired returning "{desired}"')
    ...
    def makeOpCmd(self):
        ...
        log.debug(f'makeOpCmd returning "{opcmd}"')
...
#rxData b'ISCP\x00\x00\x00\x10\x00\x00\x00\  n\x01\x00\x00\x00!1PWR00\x1a\r\n'.
        ...
        if not match:
            log.debug(f'NO MATCH on lengths')
            log.debug(f'rxData {rxData}')
            log.debug(f'opcmd  {opcmd}')

        log.debug(f'isMatchingAck returning {match}')
        return match
    ...
    def makeCmdResult(self, rxData):
        ...
        result = rxData[21:-3]
        log.debug(f'makeCmdResult returning "{result}"')
        return result
